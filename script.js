...
if (ctx) {
    ...
    defaultImg.onerror = () => {
        console.log("cat.png がサーバー内に見つからないか、読み込み中です。文字だけで先行描画します。");
        draw(); 
    };
...
if (secretTrigger && adminUi) {
    secretTrigger.addEventListener('click', () => {
        ...
        if (clickCount >= 5) {
            adminUi.classList.toggle('hidden'); // 管理画面を表示/非表示
            clickCount = 0; 
        } else {
...
// リセットボタン（一発で最初の猫のドット絵に戻す）
if (resetBtn) {
    resetBtn.addEventListener('click', () => {
        if (imageInput) imageInput.value = '';
        backgroundImage = defaultImg; // 記憶していた最初の猫に戻す
        draw();
    });
}
